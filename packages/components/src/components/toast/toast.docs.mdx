...
import Toast, { Basic, Variants, WithAction, WithCustomIcon, WithAvatar,  } from './toast.stories';
...
<Alert variant="warning" message="Deprecated">
 {'This component is going to be removed. Use Snackbar component instead'}
</Alert>
...
Set `actionLabel` and `onActionClick` to add an action to the toast component.

<Canvas of={WithAction} />

### With Custom Icon

Set `icon` to represent the icon of the toast component.

<Canvas of={WithCustomIcon} />

### With Avatar

`icon` can also be used to represent an avatar image.
